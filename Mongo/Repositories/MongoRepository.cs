...
public class MongoRepository<T> : IMongoRepository<T>
{
    ...
    {
        var filter = CreateIdFilter(entity);
    ...
    public async Task DeleteAsync(T entity)
    {
        ...
        if (Session != null)
        {
            await Collection.DeleteOneAsync(session: Session, filter: filter);
        }
        else
        {
            await Collection.DeleteOneAsync(filter: filter);
        }
    }
    ...
    private PropertyInfo GetIdProperty()
    {
        var properties = typeof(T)
            .GetProperties()
            .Where(x => x.GetCustomAttributes(typeof(BsonIdAttribute), true).Any())
            .ToArray()
            ;

        if (properties.Length == 0)
        {
            throw new Exception("Entity does not have a property decorated with [BsonId].");
        }

        if (properties.Length > 1)
        {
            throw new Exception("Entity has multiple properties decorated with [BsonId].");
        }
    ...
    private object GetIdValue(T entity)
    {
        ...
        if (id is null)
        {
            throw new Exception("Entity ID is null.");
        }

        return id;
    }

    private FilterDefinition<T> CreateIdFilter(T entity)
    {
        return FilterBuilder.Eq(IdProperty.Name, GetIdValue(entity));
    }

    private FilterDefinition<T> CreateFilterFromPredicate(Expression<Func<T, bool>> predicate)
    {
